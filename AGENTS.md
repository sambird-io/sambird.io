...
Sam Bird's personal portfolio. Single page: **Hero → About → Experience → Skills →
Contact** (`app/page.tsx` composes the sections in `components/sections/`).
...
- `site` — `name, role, company, tagline, bio, location, email, links{linkedin,github}`.
  Current positioning: **Lead Cloud Engineer @ Lloyds Banking Group** (Leeds, UK).
...
## Build → ship → deploy
1. **Test:** `npx tsc --noEmit` · `npm run lint` · `npm run build` (must emit standalone
...
2. **GitHub** (`sambird-io/sambird.io`): branch off `main` (never commit to `main`
   directly); conventional commit ending
   `Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>`;
   `gh pr create --base main` → `gh pr merge <#> --squash --delete-branch` → sync `main`.
3. **Deploy to Fly** (app **`sambird`**, region `cdg`) — in place, **never destroy**
...
`AAAA 2a09:8280:1:a164:c7:2bf4:a389:f25`; `www` → `CNAME sambird.fly.dev`. Certs:
